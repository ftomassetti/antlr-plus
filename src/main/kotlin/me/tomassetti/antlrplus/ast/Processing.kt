...
class Mapper {
    ...
    private fun toAstEntity(ptMetamodel: PtMetamodel, metamodel: Metamodel, ptEntity: PtEntity) : Entity {
        if (!metamodel.hasEntity(ptEntity.name)) {
            metamodel.addEntity(Entity(ptEntity.name, ptEntity.features.map { toAstFeature(ptMetamodel, metamodel, it) }))
        }
    ...
    private fun toAstFeature(ptMetamodel: PtMetamodel, metamodel: Metamodel, feature: PtFeature) : Feature {
        when (feature.type) {
            ...
            else -> return Containment(feature.name, toAstEntity(ptMetamodel, metamodel, ptMetamodel.byName(feature.name)), feature.multiple)
        }
    }
}
